...
setup_webui() {
    ...
    uci set luci.hppc_group=command; uci set luci.hppc_group.name='HPPC Castellan'; uci set luci.hppc_group.command='' 
    uci set luci.hppc_sync=command; uci set luci.hppc_sync.name='⚔️ 集结军队 (Sync Config)'; uci set luci.hppc_sync.command='/usr/bin/hppc sync'
    uci set luci.hppc_assets=command; uci set luci.hppc_assets.name='📚 修缮典籍 (Update Rules)'; uci set luci.hppc_assets.command='/usr/bin/hppc assets'
    uci set luci.hppc_rollback=command; uci set luci.hppc_rollback.name='🛡️ 紧急回滚 (Rollback)'; uci set luci.hppc_rollback.command='/usr/bin/hppc rollback'
    uci set luci.hppc_doctor=command; uci set luci.hppc_doctor.name='🩺 要塞诊断 (System Doctor)'; uci set luci.hppc_doctor.command='/usr/bin/hppc doctor'

    uci commit luci
    echo -e "${C_OK}✅ 部署完成！请刷新 LuCI 页面查看。${C_RESET}"
}
...
while true; do
    ...
    case $choice in
        ...
        2) 
           ...
           if [ "$confirm" == "y" ] || [ "$confirm" == "Y" ]; then
               ...
               if /etc/init.d/homeproxy restart; then
                   echo -e "${C_OK}✅ 重启成功。防御塔运作正常。${C_RESET}"
               else
                   echo ""
                   echo -e "${C_ERR}❌ 重启失败！防线可能已崩溃。${C_RESET}"
                   echo -ne "${C_WARN}⚠️  是否立即执行紧急回滚 (Rollback)? [Y/n]: ${C_RESET}"
                   read rb_confirm
                   if [ "$rb_confirm" != "n" ]; then
        ...
        3) 
           echo ""; echo -e "${C_INFO}请输入规则集名称 (如 geosite-openai):${C_RESET}"; echo -ne "输入: "; read rule_name
           if [ -n "$rule_name" ]; then sh /usr/share/hppc/modules/assets.sh --download "$rule_name"; fi
           echo ""; echo "按回车返回..."; read 
           ;;
